...
public class DatabaseHelper_pwd extends SQLiteOpenHelper {
    // 데이터베이스 이름이다 (테이블 이름과 무관)
    public static String NAME = "password.db";
    public static int VERSION = 1;

    // 내가 SQLite를 실행하려고 하는 context를 설정해준다. 그걸 경우, 백그라운드에 있는 SQLite가 자동으로 설정된다.
    public DatabaseHelper_pwd(Context context){
    ...
    public void onCreate(SQLiteDatabase sqLiteDatabase) {
        String sql = "create table if not exists password_table (pwd PRIMARY KEY)";
